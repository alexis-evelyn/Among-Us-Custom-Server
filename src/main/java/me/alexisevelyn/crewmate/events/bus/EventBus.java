...
public class EventBus {
    ...
    public void unregister(Object clazz) { listeners.values().forEach(it -> it.removeIf(listener -> listener.instance == clazz)); }
    ...
    public void post(Event event) {
        ...
        for (EventHandlerContainer container : this.listeners.getOrDefault(event.getClass(), new CopyOnWriteArrayList<>())) {
            if (cancellable && ((EventCancellable) event).isCancelled() && container.cancellable) return;
